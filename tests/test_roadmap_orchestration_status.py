...
def test_active_docs_identify_current_next_pr_and_do_not_bind_old_progress() -> None:
    ...
    current_next = (
        "PR-51: versioned nonidentity ParameterRecord conversion registry"
    )
...
def test_build_first_queue_defers_validation_without_deleting_it() -> None:
    ...
    assert "complete in the current checkout for the bounded loaded-source bridge scope" in status
...
def test_validation_data_gate_stays_deferred_and_not_complete() -> None:
    ...
    assert "selected PR-51 follow-up is a versioned nonidentity" in gate
