...
class CalibrationSplit:
    """Train/validation split indices for each dataset measurement."""
...
def calibrate_configured_model(
    *,
    ...
    source_config_path = Path(model_config)
    ...
    config = load_model_config(source_config_path)
    ...
    def predict(parameters: ParameterSet) -> Mapping[str, Quantity]:
        result = _run_with_parameters(source_config_data, source_config_path, parameters, symbols)
    ...
    fit_result = fit_least_squares(
        ...
        validation_indices=(),
        calibration_source=f"Synthetic configured calibration against dataset {dataset_obj.dataset_id}.",
        max_nfev=max_nfev,
    )
    final_result = _run_with_parameters(source_config_data, source_config_path, fit_result.fitted_parameters, symbols)
...
def _build_split(dataset: ExperimentDataset, split: Mapping[str, Any] | None) -> CalibrationSplit:
    if split is None:
        ...
        return CalibrationSplit(method="none", train_indices=train, validation_indices=validation)
    ...
    for series in dataset.measurements:
        ...
        n_train = max(1, min(n_points - 1, int(np.floor(n_points * train_fraction))))
        train_indices[series.measurement_id] = order[:n_train]
        validation_indices[series.measurement_id] = order[n_train:]
        if set(train_indices[series.measurement_id]).intersection(validation_indices[series.measurement_id]):
            raise ConfiguredCalibrationError("Train and validation indices must not overlap.")
    return CalibrationSplit(method="by_time", train_indices=train_indices, validation_indices=validation_indices)
