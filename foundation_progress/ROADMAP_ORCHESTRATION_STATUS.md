...
| PR-19 | PRODUCT-001 degradation-rate quicklook/report ergonomics | current next; partial for researcher-facing virtual-experiment outputs | Improve degradation-rate inspection and report/quicklook ergonomics from existing standard output tables. No hidden notebook science, new biology, solver behavior, validation data, calibration, empirical comparison, inferred environment responses, posterior uncertainty claim, solver/model changes, or silent fallback constants. |
| PR-20 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
