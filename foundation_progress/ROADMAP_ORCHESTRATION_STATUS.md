...
| PR-21 | THERMO-003 explicit thermodynamic-summary report ergonomics | current next; THERMO-003 remains partial | Improve report or inspection ergonomics for existing explicit configured-output thermodynamic diagnostics. No inferred activities, inferred reaction quotients, inferred concentrations, redox model, electron-balance model, validation data, empirical comparison, solver-time enforcement, hidden notebook science, or silent fallback constants. |
| PR-22 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
