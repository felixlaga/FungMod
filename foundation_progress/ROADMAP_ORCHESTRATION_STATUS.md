...
| PR-13 | THERMO-003 entropy-production-rate notebook coverage | current next; partial for thermodynamic/entropy constraints | Extend the configured-output thermodynamics notebook to inspect existing entropy-production-rate JSON/CSV fields from explicit configured metadata. No new equations, inferred thermodynamics, inferred activities/Q/concentrations, redox model, electron-balance model, validation claim, or solver-time enforcement. |
| PR-14 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
