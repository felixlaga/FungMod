...
| PR-10 | PRODUCT-001 report-folder index/navigation | current next; partial for report/output ergonomics | Add an opt-in report-folder `index.html` linking the existing Markdown report, optional HTML report, standard CSV tables, output manifest, and quicklook figures when present. No new biology, solver behavior, validation data, calibration, empirical comparison claims, scientific reinterpretation, or hidden report logic. |
| PR-11 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
