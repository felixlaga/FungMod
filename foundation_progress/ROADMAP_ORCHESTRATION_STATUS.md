...
| PR-16 | PRODUCT-001 uncertainty-band output ergonomics | current next; partial for researcher-facing virtual-experiment outputs | Improve how existing uncertainty/range information is surfaced in standard outputs or reports without adding validation data, calibration, empirical comparison, inferred environment responses, or silent fallback constants. |
| PR-17 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
