...
| PR-18 | PRODUCT-001 trajectory-quantile example and quicklook ergonomics | current next; partial for researcher-facing virtual-experiment outputs | Improve trajectory-quantile inspection and quicklook ergonomics from existing standard output tables. No hidden notebook science, new biology, solver behavior, validation data, calibration, empirical comparison, inferred environment responses, or silent fallback constants. |
| PR-19 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
