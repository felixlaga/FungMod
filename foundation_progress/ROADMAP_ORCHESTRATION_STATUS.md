...
| PR-23 | PRODUCT-001 provenance/limitations report example notebook | current next; PRODUCT-001 remains partial | Add a public-API example notebook that writes reports and inspects the provenance/limitation decision summary and existing decision-support table links. No validation data, calibration, empirical comparison, inferred environment response, solver/model behavior, schema change, or hidden notebook science. |
| PR-24 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
