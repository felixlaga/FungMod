...
| PR-17 | PRODUCT-001 trajectory-quantile output ergonomics | current next; partial for researcher-facing virtual-experiment outputs | Derive trajectory-level quantile or band outputs from existing sample time-series rows with explicit guardrails. No new biology, solver behavior, validation data, calibration, empirical comparison, inferred environment responses, or silent fallback constants. |
| PR-18 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
