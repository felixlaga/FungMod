...
| PR-09 | PRODUCT-001 HTML report wrapper | current next; partial for report/output ergonomics | Add a tiny browser-viewable HTML sidecar around the existing Markdown report and standard output folder links. No new biology, solver behavior, validation data, calibration, empirical comparison claims, or hidden report logic. |
| PR-10 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
