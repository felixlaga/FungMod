...
| PR-24 | VALIDATION-DATA-001 first real time-course dataset and model comparison | current next only if source-backed observations satisfy the active evidence gate; otherwise deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful and the evidence gate is satisfied. Validation, calibration, and empirical comparison claims require real observations. |
