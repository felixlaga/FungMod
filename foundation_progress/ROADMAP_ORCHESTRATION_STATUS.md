...
| PR-08 | PRODUCT-001 virtual-experiment report writer | current next; partial for report/output ergonomics | Add a researcher-facing report writer over existing standard output tables. No new biology, solver behavior, validation data, calibration, or empirical comparison claims. |
| PR-09 | VALIDATION-DATA-001 first real time-course dataset and model comparison | deferred | Add sourced observations only after simulator outputs are mature enough that comparison is meaningful. Validation, calibration, and empirical comparison claims require real observations. |
