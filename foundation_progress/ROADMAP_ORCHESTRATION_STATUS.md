...
| PR-03 | VALIDATION-DATA-001 first real time-course dataset and model comparison | blocked on source-data criteria | Add a sourced real dataset and comparison workflow without overclaiming validation. This remains current-next until an ingestable source is found. |
...
| VALIDATION-DATA-001 first real time-course dataset | blocked pending ingestable source | `foundation_progress/VALIDATION_DATA_001_FIRST_TIMECOURSE.md`, `tests/test_dataset_candidate_review.py`, `tests/test_roadmap_orchestration_status.py` | Do not fabricate data, digitize unresolved figures, ingest blocked candidate reviews, or claim validation without sourced observation rows, units, extraction/transcription provenance, uncertainty policy, preprocessing notes, and explicit comparison limits. |
